...
public:
    ...
    explicit SearchServer(const StringContainer& stop_words)
        ...
        for (const auto word : stop_words_) {
            if (!IsValidWord(word)) {
                throw invalid_argument("your stop words contains special symbols"s);
            }
        }
    }
    ...
    int GetDocumentId(int index) const {
        if (index < 0 || index >= get_ID_for_index.size()) {
            throw out_of_range("your index is lower than zero or bigger than available number of documents"s);
        }
        return get_ID_for_index.at(index);
    }
...
/*int main() {
    ...
    if (!search_server.AddDocument(-1, "пушистый чай и модный ошейник"s, DocumentStatus::ACTUAL, {1, 2})) {
    ...
    tuple<vector<string>, DocumentStatus> abc;
    if (search_server.MatchDocument("кот и", 1, abc)) {
        ;
    } else {
        cout << "Ошибка в поисковом запросе"s << endl;
    }
